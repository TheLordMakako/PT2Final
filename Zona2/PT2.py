...
path="PT2.py"        #Esta variable se tiene que cambiar dependiendo del codigo
CDMX='19.332233,-99.248878,9km'  # geocode de longitud y latidud, circunferencia de 15 kilometros de radio
...
def PrimeraLectura(tema):
    #Valor C se cambia la numeración dependiendo del script a usarse
    carpeta=ruta+"C2/" + tema
    os.makedirs(carpeta, exist_ok=True)
    NomArchivo=carpeta+'/C2_'+tema+'.json'
    global data
    try:
...
def GuardadoFinal(tema,data):
    carpeta=ruta+"C2/" + tema  #Valor de C se cambia la numeración dependiendo del script a usarse
    NomArchivo=carpeta+'/C2_'+tema+'.json'    
    try:
        with open(NomArchivo, 'w') as file:
...
def main():
    ...
    print("Termine todo el script PT2 exitosamente el: "+time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()))
