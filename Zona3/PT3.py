...
path="PT3.py"        #Esta variable se tiene que cambiar dependiendo del codigo
CDMX='19.374214,-99.110017,10km' # geocode de longitud y latidud, circunferencia de 15 kilometros de radio
...
def PrimeraLectura(tema):
    ...
    carpeta=ruta+"C3/" + tema
    os.makedirs(carpeta, exist_ok=True)
    NomArchivo=carpeta+'/C3_'+tema+'.json'
    global data
    try:
...
def GuardadoFinal(tema,data):
    carpeta=ruta+"C3/" + tema #Valor de C se cambia la numeración dependiendo del script a usarse
    NomArchivo=carpeta+'/C3_'+tema+'.json'    
    try:
        with open(NomArchivo, 'w') as file:
...
def main():
                        ...
                        if ( tweetIn == 'tuits' or tweet.id != tweetIn["id"]):
                            ...
                            try:
                                ...
                                data['tuits'].append({
                                                ...
                                                'favorited': tweet.favorited,
                                                'retweeted': tweet.retweeted,
                                                'zona':'C3'    #este cambia dependiendo del circulo en donde se este trabajando
                                                })
    ...
    print("Termine todo el script PT3 exitosamente el: "+time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()))
    input("da un click para terminar")
    time.sleep(120)           
